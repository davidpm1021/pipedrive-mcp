...
@tool("custom_fields")
async def list_org_fields_from_pipedrive(
    ctx: Context,
    limit_str: Optional[str] = "100",
    start_str: Optional[str] = "0",
) -> str:
    """Lists organization fields (standard and custom) from Pipedrive CRM.

    Returns the field metadata for organizations: id, key (the API key used in
    ...
    before creating or updating an organization.
    ...
    Example:
    ```
    list_org_fields_from_pipedrive(limit_str="200")
    ```
    ...
    Returns:
        JSON formatted response with the organization field list and pagination info, or error message
    """
