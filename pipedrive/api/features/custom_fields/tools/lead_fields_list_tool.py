...
@tool("custom_fields")
async def list_lead_fields_from_pipedrive(
    ctx: Context,
    limit_str: Optional[str] = "100",
    start_str: Optional[str] = "0",
) -> str:
    """Lists lead fields (standard and custom) from Pipedrive CRM.

    Returns the field metadata for leads: id, key (the API key used in
    ...
    on leads — in particular the API keys you need to pass via the
    `custom_fields` parameter on create_lead_in_pipedrive and
    ...
    Example:
    ```
    list_lead_fields_from_pipedrive(limit_str="200")
    ```
    ...
    Returns:
        JSON formatted response with the lead field list and pagination info, or error message
    """
